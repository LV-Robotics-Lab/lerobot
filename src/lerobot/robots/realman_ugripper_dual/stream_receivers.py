...
# 工作区根目录: .../lerobot_tactile_ws
# 本文件路径: .../lerobot_tactile_ws/lerobot/src/lerobot/robots/realman_ugripper_dual/stream_receivers.py
_WS = Path(__file__).resolve().parents[5]
_FISH_CLIENT_DIR = _WS / "ugripper" / "fish_camera_client"
_SDK_DIR = _WS / "SDK_Publish_1.2.3"
...
def _tactile_worker(
    ...
    """触觉工作进程: dmrobotics Flux getDepth+getDeformation2D -> (H,W,3) uint16 RGB。

    通道: B=depth, G=deform_x, R=deform_y。
    SDK 输出的 depth/deformation 均为 float32, 用定点编码存成 uint16:
        depth  -> round(depth  * 1000)            (depth>=0, 量级 mm)
        deform -> round(deform * 1000 + 30000)    (有正负, +30000 偏置后非负)
    解码: depth = v / 1000;  deform = (v - 30000) / 1000。
    (与参考实现 ugripper/unified_client.py 一致, 用 deformation 而非 shear。)
    ...
    while not stop.is_set():
        ...
        try:
            ...
            while not stop.is_set():
                ...
                nd = depth * 1000.0                                            # (H, W)
                ng = deform * 1000.0 + 30000.0                                 # (H, W, 2)
                rgb = np.concatenate([nd[..., np.newaxis], ng], axis=-1)       # (H, W, 3)
                rgb16 = np.clip(np.round(rgb), 0, 65535).astype(np.uint16)
