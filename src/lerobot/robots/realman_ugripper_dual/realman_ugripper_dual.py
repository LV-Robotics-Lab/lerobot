...
数据格式 (每条启用的手臂, side ∈ {left, right}):
    ...
    observation.images:
        ...
        {side}_cam_finger0     (240, 320, 3)   uint16 [depth, deform_x, deform_y]
        {side}_cam_finger1     (240, 320, 3)   uint16
    action:
        {side}_main_joint1..7  (float, 弧度)
...
# 添加 Robotic_Arm SDK 路径 (.../lerobot_tactile_ws/lerobot)
...
class RealmanUGripperDual(Robot):
    ...
    def __init__(self, config: RealmanUGripperDualConfig):
        ...
        self._empty_tactile = np.zeros((config.tactile_height, config.tactile_width, 3), dtype=np.uint16)
    ...
    def _import_sdk(self):
        if self._arm_sdk is None:
            try:
                ...
                raise ImportError(
                    f"无法导入睿尔曼 SDK (Robotic_Arm)。请确认其在工作区 lerobot/ 下。原始错误: {e}"
                )
    ...
    def _connect_gripper(self, side: str, arm: _ArmDevices) -> None:
        ...
        try:
            ...
            if not gripper.init_gripper(itinerary_override=itinerary):
                ...
                return
            # grip_init 自标定结束停在接近夹紧位 (move_to_pos(100)), 张开复位到初始位,
            # 否则每次连接/推理开始夹爪都是夹紧状态。
            gripper.move_norm(1.0)
            arm.gripper = gripper
        except Exception as e:
